...
public interface DmsEmpTargetRankingBranchDao  extends CrudRepository<DmsEmployeeTargetRankingBranch, Integer> {
	  @Query(value = "SELECT * FROM dms_emp_target_ranking_branch WHERE emp_id=?1 and branch_id=?2", nativeQuery = true)
			Optional<DmsEmployeeTargetRankingBranch> findByEmpIdAndBranchId(Integer empId,Integer branchId);
